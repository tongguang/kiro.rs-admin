...
接入 Codex：在 `~/.codex/config.toml` 配一个自定义 provider，`base_url` 指向 `http://<host>:<port>/v1`、`wire_api = "responses"`、`env_key` 填客户端 Key 对应的环境变量。Codex 会向 `base_url` + `/responses` 发起请求。请求里的模型名**原样透传**给 Kiro 后端（Kiro 有真实 gpt-5.6 系列可用）；需要别名（如 `gpt-5.5` → `claude-opus-4.8`）时用 Admin UI / `model_mappings.json` 配置映射，首次启动已默认 seed `gpt-5.5` / `gpt-5.4` → `claude-opus-4.8` 两条。带 `{"type":"web_search"}` 工具时，服务端内部执行联网搜索并把结果喂回模型合成答案（不向客户端下发原始搜索块）。

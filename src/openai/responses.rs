...
pub struct ResponsesRequest {
    ...
    /// fork 扩展：stateful 续接（上游无 stateless-only 之外的存储语义）。
    /// 命中本地 LRU 存储时把历史链拼到本次 input 前，模拟 OpenAI 服务端会话状态。
    #[serde(default)]
    pub previous_response_id: Option<String>,
    /// fork 扩展：默认 true；显式 false 时本响应不入续接存储。
    #[serde(default)]
    pub store: Option<bool>,
    /// fork 扩展：客户端元数据，原样回显到响应对象。
    #[serde(default)]
...
// ==================== previous_response_id 存储（fork 扩展） ====================

/// 单个已存响应：完整响应对象（GET 回显用）+ 续接链（历史 input items 与
...
fn input_as_items(input: &Value) -> Vec<Value> {
    match input {
...
pub async fn post_responses(
    ...
    if !chained_items.is_empty() {
        chained_items.extend(input_as_items(&req.input));
        req.input = Value::Array(chained_items);
    }
    // 存链用的完整输入（含续接前缀），仅 store=true 时保存
    let full_input_items: Vec<Value> = input_as_items(&req.input);
    ...
    let mut body = build_responses_object_with_config(&parsed, &tool_kinds, &response_config);
    body["previous_response_id"] = json!(previous_response_id);
    if let Some(md) = &request_metadata {
        body["metadata"] = md.clone();
    }
    if store_response {
        let mut items = full_input_items;
...
fn responses_to_anthropic(
    ...
    Ok((
        MessagesRequest {
            ...
            output_config,
            metadata,
            // fork 的 websearch_loop 由该内部标记触发；客户端声明了 hosted
            // web_search（且未显式 tool_choice:none）即注入原生工具并进 loop
            force_web_search_loop: hosted_web_search_declared && !tool_choice_none,
        },
        tool_kinds,
    ))
}
